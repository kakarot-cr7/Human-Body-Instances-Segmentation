...
from Training import load_dataset, create_dir, get_colormap
...
def save_results(image, pred, save_image_path): 
    ...
    cat_images = np.concatenate([image, line, pred], axis=1)
...
if __name__ == "__main__":
    ...
    create_dir("results2.0")

    """ Hyperparameters """
    IMG_H = 320
    IMG_W = 416
    NUM_CLASSES = 20
    dataset_path = "E:/Instance Segmentation/instance-level_human_parsing"
    model_path = os.path.join("files", "model2.0.h5")
    ...
    for path in tqdm(test_images,total= len(test_images)):
        ...
        save_image_path = f"results2.0/{name}.png"
